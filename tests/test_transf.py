...
if __name__ == "__main__":
    ...
    # Transformer inplace
    tr = Transformer_MaskFeats(transformer_type="SqrtTransform", features_to_transform=[True, False, False])
    data_tr_2 = tr.fit_transform(data_norm[['VDSolar','TSolarRL','TSolarVL']])

    tr = Transformer_MaskFeats(transformer_type="Boxcox", transformer_params={'omit_zero_samples': True}, features_to_transform=[True, False, False])
    ...
    tr = Transformer_MaskFeats(transformer_type="PolynomialExpansion", features_to_transform=[True, True, False], mask_type='MaskFeats_Expanded')
    data_tr_4 = tr.fit_transform(data[['VDSolar', 'TSolarRL', 'TSolarVL']])
    print(tr.get_feature_names_out(['VDSolar', 'TSolarRL', 'TSolarVL']))
